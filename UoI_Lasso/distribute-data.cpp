...
lasso_sweep (MatrixXf X, VectorXf y, VectorXf lambda, float train_frac, int n_bootstraps, bool use_admm, bool debug, int MAX_ITER, float RELTOL, float ABSTOL, float rho, MPI_Comm comm_sweep)
{
  ...
  if(rank_sweep==0)
    estimates.setZero(n_bootstraps * n_lambdas, n_features);
...
int main(int argc, char** argv) {
...
#ifdef ALPHAGRID
  float alpha_;
  tie(lambdas,alpha_) = alpha_grid(X_, y_, 1.0, lasso_init.n_lambdas, 1e-3, nrows);
    if ( (rank == 0) && lasso_init.verbose)
      cout << "(2) Beginning model selection around " << alpha_ << "." << endl;
  ...
  if(rank==0 && lasso_init.debug) cout << "lambdas" << endl;
  ...
  free(B);
    ...
    //MatrixXf estimates_cboot_root, 
    MatrixXf scores_cboot_root, scores_clam_root, scores_mc;
    ...
    if(rank==0)
      cout << "Exploring penalty region centered at " << lambda_max << "." << endl;
  ...
  if(rank==0 &&  lasso_init.debug)
    print_vector( lambdas, "./debug/lambdas.txt");
  ...
  VectorXf y_sel(local_rows);
  X_sel = B_sel.block(0, 0, B_sel.rows(), B_sel.cols()-1);
  y_sel = B_sel.rightCols(1);
  ...
  tie (estimates_dense, scores_dense,lass_time, lass_comm)  =
    lasso_sweep (X_sel, y_sel, lambda_dis, lasso_init.train_frac_sel, (lasso_init.n_boots_sel/lasso_init.n_groups),
  ...
  if ( (rank == 0) && lasso_init.debug) {
    cout << "size of estimates_dense " << estimates_dense.rows() << " ," << estimates_dense.cols() << endl;
    cout << "1 element of estimateds" << estimates_dense(0,0) << endl;  
    cout << "1 element of scores_dense" << scores_dense(0,0) << endl << endl;
  ...
  if(rank_g == 0)
    ...
    if ( (rank == 0) && lasso_init.debug)
      print_matrix( estimates_l1, "./debug/estimates_l1.txt");
